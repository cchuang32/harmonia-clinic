...
`assets/img/` 裡放的是**處理過的上線版**（已縮小、已清除 EXIF）。
沒處理過的原始檔——手機直出的照片、AI 生成的原始輸出、設計師給的高解析檔——
一律放在 repo **外面**：

```
~/Pictures/君禾診所-原圖/
    衛教插圖/    診所空間/    醫師與證書/    logo與識別/
```

檔名用「上線後的檔名 + `_原圖_寬x高`」，例如
`bma-clot-fibrin_原圖_2752x1536.jpeg` 對應上線的 `assets/img/bma-clot-fibrin.jpg`。

**為什麼不放在 repo 裡**：一是原圖動輒好幾 MB，進了 git 會把版本庫撐大而且刪不掉；
二是這個 repo 是公開的，手機照片內含 GPS 座標，推上去等於公開診所或住家位置。

**為什麼要留**：上線版是為了網頁載入速度壓過的，回不去。哪天要印海報、
做大圖或換一個需要更高解析度的版面，手上只有 1200 px 的上線版就不夠用了。
...
> **改文章不會影響排序。**
> 列表一律照「發佈日」排，新的在前；最後更新日只是顯示用的標記。
> 也就是說，你今天回頭修八月那篇，它**不會**跳到最前面，還是待在原來的位置。
> 這是刻意的——不然讀者會以為有新文章，點進去卻是看過的舊文。
> 要讓一篇文章排到前面，只能改它的 `date`（發佈日）。
...
| 檔案 | 用在哪 |
| --- | --- |
| `assets/img/logo-banner.jpg` | 首頁最上方的 banner（正方形，**等比例顯示，不裁切也不拉伸**） |
| `assets/img/logo-mark.png` | 每一頁左上角的小標誌（透明背景，只有圖案） |
| `assets/img/favicon.svg` | 瀏覽器分頁的小圖示 |

全站配色以 logo 為基準，定義在 `assets/css/style.css` 最上面的 `:root`：

- `--brand: #9cab80` — **logo 背景的橄欖綠**。首頁 banner 兩側的留白就是靠它跟圖片無縫接起來，
  所以**換 banner 圖時，如果新圖底色不一樣，一定要同時改這個值**，否則會看到一條色差。
- `--green-900 / 700 / 500 / 100 / 050` — 同色相的深淺版本，深色供文字使用（對比度都通過無障礙標準）。
- `--accent: #d18f40` — 琥珀橘，**只做裝飾**（小標籤前的短線、可點卡片 hover 的邊框、頁尾標題底線）。
- `--gold-600: #8a6a2f` — 需要用暖色的**文字**（小標籤、醫師頭銜）一律用這個深金。

目前各色用在哪：

| 元素 | 顏色 |
| --- | --- |
| 首頁 banner 底、banner 兩側留白 | `--brand` |
| 標題、頁尾底 | `--green-900` |
| 按鈕、連結、選中的頁籤 | `--green-700` |
| 邊框、hover | `--green-500` |
| 圖示底、區塊淡底 | `--green-100` / `--green-050` |
| 小標籤前的短線、卡片 hover 邊框、頁尾標題底線 | `--accent` |
| 小標籤與醫師頭銜的字 | `--gold-600` |

**兩條不能破的規則**（文字要看得清楚，對比度至少 4.5）：

1. **`--accent` 不能拿來寫字**。亮橘放白底只有 2.72，白字放橘底也是 2.72。要暖色的字就用 `--gold-600`。
2. **`--accent` 不能貼在 `--brand` 上**。兩者亮度幾乎相同（0.336 對 0.378），對比只有 1.11，會糊成一片。

另有備用的 `.section--brand`：想讓某個區塊整片用品牌綠，把該 `<section>` 的
`section--paper` 換成 `section--brand` 即可（**這個底色上只能放深色字**，配套規則已寫好）。
...
>
> 原本另有一份 GitHub Pages 備援副本（`cchuang32.github.io/harmonia-clinic/`），
> 2026-09-22 已關閉並刪除 `.github/workflows/deploy.yml`：它沒有實際用途，
> 偶爾 GitHub 端出錯還會跳出失敗通知，讓人誤以為官網壞了。
> `tools/build.mjs` 仍支援 `BASE_PATH`，但目前沒有地方用到。
