...
const facilities = [
  ...
  },
  {
    ...
    photo: '/assets/img/facility-shockwave-room.jpg',
    photoAlt: '君禾診所治療室裡的體外震波治療儀：推車上放著震波主機與治療探頭，檯面上備有多顆不同尺寸的傳導頭，左側為診療床',
...
const pillars = [
  {
    ...
    // 空間照片。留空 → 不顯示，版面照常。全部都已裁成 3:2，並清除 GPS 等中繼資料。
    // 順序照病人進門後的動線：門口 → 大門 → 櫃檯 → 候診區 → 通往診間的走道，最後是空氣清淨機。
    // 六張排成 2×3；之後增減照片時，盡量維持偶數張，才不會有一張孤零零落單。
    // ★ 門口照片只寫「有斜坡板、門寬 120 公分」，不要寫門口與人行道的高低差（網站主人的決定）。
    photos: [
      {
        src: '/assets/img/space-entrance-ramp-wide.jpg',
        alt: '君禾診所門面：上方是淺綠色的君禾診所招牌，中間的自動玻璃門敞開，門口地上橫鋪著一塊黑色斜坡板，門內可見櫃檯與木質牆面，左側玻璃上貼著門診時間表與 QR 碼。',
