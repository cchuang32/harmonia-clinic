...
// ---------------------------------------------------------------------------
// 常見問題（文字由診所提供，取自「自體骨髓注射 Q&A」文件）。
// 寫法固定：問句當標題，第一句先給答案，再展開細節——這是 AI 搜尋最容易
// 抓成引用片段的格式。往陣列裡加一組就會多一個類別，並自動進結構化資料。
// ---------------------------------------------------------------------------
const faqGroups = [
  {
    title: '關於「抽骨髓」的擔心',
    lead: '聽到要從骨盆抽骨髓，很多人第一個反應是往後退一步。這幾題是門診裡最常被問到的。',
    items: [
      { q: '抽骨髓是不是很痛？',
  ...
  {
    title: '效果與合理期待',
    lead: '這一段講的是「可以期待什麼」，也講「不能期待什麼」。',
    items: [
      { q: '多久開始有效？',
        a: '不會馬上見效。它不是消炎止痛針，而是提供修復所需的原料，多數人是在數週到數個月之間，慢慢感覺到疼痛與活動度的改善。如果期待的是「打完就不痛」，這個期待需要先調整。' },
      { q: '效果可以維持多久？',
        a: '因人而異。病灶嚴重度、年齡、體重、活動量與整體健康狀況都會影響，有些人維持較久，有些人改善幅度有限。治療後的日常保養，往往才是能不能維持得久的關鍵。日常保養實際的建議包括良好習慣、伸展放鬆、及運動訓練。' },
      { q: '需要注射幾次？',
        a: '多數人會先安排一次，觀察二到四週，看看反應如何，再決定要不要追加。是否需要第二次，會依改善程度、病灶狀況與治療目標一起討論，不需要在一開始就綁定一整套療程。' },
      { q: '年齡大還有效嗎？',
        a: '年齡確實會影響骨髓中細胞的數量與活性，但目前沒有一個絕對的年齡上限。臨床上更關鍵的，通常是關節破壞到什麼程度、肌力狀況與生活型態，而不是身分證上的數字。' },
      { q: '能不能讓軟骨重新長回來？',
        a: '目前的研究還無法證實 BMA 可以讓已經磨損的軟骨長回來。比較合理的期待，是減輕疼痛、改善關節功能與活動能力。任何宣稱「能讓軟骨再生」的說法，都建議保留一點空間看待。' },
      { q: '我要怎麼知道有沒有效？',
        a: '主要看你自己的感受：疼痛分數、一次能走多遠、上下樓梯與蹲下起身順不順、晚上會不會痛醒。建議治療前先把這些記下來，回診時比較才客觀。影像上的變化通常比症狀改善慢很多，不是唯一指標。' },
      { q: '可以避免人工關節或手術嗎？',
        a: '對還沒到末期的患者，它有機會延緩或減少手術的需要，但不能保證。如果關節已經明顯變形、磨損嚴重，或有複雜的結構性問題需要處理，手術仍然是比較合理的選擇，醫師會在評估時據實說明。' },
      { q: '治療失敗後還能接受其他治療嗎？',
        a: '可以。BMA 使用的是你自己的組織，不會破壞關節結構，也不會排擠掉後續的選項。之後要做其他注射治療、體外震波、復健療程或手術評估，都不受影響。' },
    ],
  },
];
...
<section class="section section--tint">
  <div class="wrap measure">
    ...
    ${faqGroups.map((g) => `<div class="faq-group">
      <h3 class="faq-group-title">${esc(g.title)}</h3>
      ${g.lead ? `<p class="faq-group-lead">${esc(g.lead)}</p>` : ''}
      <div class="faq">
        ${g.items.map((f) => `<div class="faq-item">
          <h4 class="faq-q">${esc(f.q)}</h4>
